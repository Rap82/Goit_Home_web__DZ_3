...
def replace_files ( root_path , path_file ) :   # Функція переносить файли з папки де вони є в папку з відповідною категорією. 
    ...
    new_path_dir.mkdir(exist_ok=True,parents=True)  # Якщо папка не існує то створити її. Примітка тесаме що закоментований код вище тільки одним рядком.
...
def sort_dir ( root_path , path ) : #  Функція отримує два парметри. Перший шлях кореневий (шлях до папки яку сортуємо ) і Другий шлях поточної папки оскільки функція буде викликатись рекурсивно то він буде містити  шлях до поточної папки яку перевірямо в даий момент.
    ...
    try :  
     threads = [] # список в який будемо добавляти наші потоки
     for item in path.iterdir() : 
        ...
        if item.is_dir() : 
            ...
            if not item.stem in SORT_CATEORY :
                thread = Thread(target=sort_dir, args=(root_path, item)) # Формуємо потік за допомогою класу *Thread з відповідними параметрами
                                                                         # Де *target=sort_dir - пердаєм імя функції яку будемо заганяти в потоки
                                                                         # *args=(root_path, item)- аргумент які будемо предавати в цю функцію.
                thread.start() # *екзепляр_класу_Thread.start() -метод який запускає поточний потік .
                threads.append(thread) # Добавляємо поточний екзепляр(потік) внаш список *threads
                logging.info(f"{thread} --Папка {item.stem}") # Аналог *print для процесів. Виводимо в каонсоль поточний потік і імя папки який він опрацьовує.
                sleep(1) # Gfepfd 1 сек щоб можна було в консолі прочитати який потік яку папку опрацьовує
            for thread in threads: # В цикл в якому очікуємо коли всі потоки коректно завершаться.
                thread.join() # Метонд який очікує завершення потоку і закриває його.
                
                # sort_dir ( root_path , item ) # наразі закоментовано оскільки сам виклки функції реалізовани вище через багатопоточність 
...
def main(path_str: str = None) -> str:
    ...
    try:
        ...
        return f"Ви не вели шляху до папки яку будемо сортувати !!!!"
    if not path.exists():
        return f"Вказаний Вами шлях '{path}' нажаль не існує !!!.\nСпробуйте ще раз."
    
    sort_dir ( root_path , path )
    
    return f"Всі Ваші папки успішно посортовані !!!\nПосортована папки знаходиться ось тут {path}\n\
        Список відсортованих файлів 'file_info_in_dir.txt'"
